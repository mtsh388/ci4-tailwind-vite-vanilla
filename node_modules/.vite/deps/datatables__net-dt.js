...
var DataTable = function(selector, options) {
	...
	this.each(function() {
		var oInit = len > 1 ? _fnExtend({}, options, true) : options;
		var i = 0, iLen;
		...
		if (oInit.on && oInit.on.options) _fnListener($this, "options", oInit.on.options);
		...
		if (oInit.on) Object.keys(oInit.on).forEach(function(key) {
			...
			$(rowOne[0]).children("th, td").each(function(i, cell) {
				...
				if (col.mData === i) {
					...
					if (sort !== null || filter !== null) {
						col.mData = {
							_: i + ".display",
							sort: sort !== null ? i + ".@data-" + sort : void 0,
							type: sort !== null ? i + ".@data-" + sort : void 0,
							filter: filter !== null ? i + ".@data-" + filter : void 0
						};
		...
		if (oSettings.caption) {
			if (caption.length === 0) caption = $("<caption/>").prependTo($this);
		...
		if (caption.length) oSettings.colgroup.insertAfter(caption);
		else oSettings.colgroup.prependTo(oSettings.nTable);
		...
		if (tfoot.length === 0) tfoot = $("<tfoot/>").appendTo($this);
		oSettings.nTFoot = tfoot[0];
		oSettings.aiDisplay = oSettings.aiDisplayMaster.slice();
		oSettings.bInitialised = true;
		var oLanguage = oSettings.oLanguage;
		$.extend(true, oLanguage, oInit.oLanguage);
		if (oLanguage.sUrl) $.ajax({
			dataType: "json",
			url: oLanguage.sUrl,
...
var _htmlNumeric = function(d, decimalPoint, formatted, allowEmpty) {
	...
	return !_isHtml(d) ? null : _isNumber(_stripHtml(d), decimalPoint, formatted, allowEmpty) ? true : null;
};
...
DataTable.util = {
	...
	*/
	set: function(source) {
		if ($.isPlainObject(source)) return DataTable.util.set(source._);
	...
	*/
	get: function(source) {
		...
		} else if (source === null) return function(data) {
			return data;
		};
...
function _fnCamelToHungarian(src, user, force) {
	...
	$.each(user, function(key) {
		...
		if (hungarianKey !== void 0 && (force || user[hungarianKey] === void 0)) if (hungarianKey.charAt(0) === "o") {
...
function _fnCompatOpts(init) {
	...
	if (typeof init.bSortCellsTop === "boolean") init.titleRow = init.bSortCellsTop;
	var searchCols = init.aoSearchCols;
	if (searchCols) {
		for (var i = 0, iLen = searchCols.length; i < iLen; i++) if (searchCols[i]) _fnCamelToHungarian(DataTable.models.oSearch, searchCols[i]);
	}
...
function _fnCompatCols(init) {
	...
	if (typeof dataSort === "number" && !Array.isArray(dataSort)) init.aDataSort = [dataSort];
}
/**
...
function _fnColumnOptions(oSettings, iCol, oOptions) {
	...
	if (oOptions !== void 0 && oOptions !== null) {
		...
		if (oOptions.mDataProp !== void 0 && !oOptions.mData) oOptions.mData = oOptions.mDataProp;
		...
		if (oOptions.iDataSort !== void 0) oCol.aDataSort = [oOptions.iDataSort];
...
function _fnColumnIndexToVisible(oSettings, iMatch) {
	var iPos = _fnGetColumns(oSettings, "bVisible").indexOf(iMatch);
...
function _fnVisibleColumns(settings) {
	...
	var vis = 0;
	if (layout.length) {
		for (var i = 0, iLen = layout[0].length; i < iLen; i++) if (columns[i].bVisible && $(layout[0][i].cell).css("display") !== "none") vis++;
	}
	return vis;
}
/**
		...
		if (!col.sType && col._sManualType) col.sType = col._sManualType;
			...
			for (j = 0, jen = types.length; j < jen; j++) {
				...
				for (k = 0, ken = data.length; k < ken; k++) {
					if (!data[k]) continue;
					if (cache[k] === void 0) cache[k] = _fnGetCellData(settings, k, i, "type");
	...
	if (aoColDefs) for (i = aoColDefs.length - 1; i >= 0; i--) {
		...
		for (j = 0, jLen = aTargets.length; j < jLen; j++) {
			...
			if (typeof target === "number" && target >= 0) {
				while (columns.length <= target) _fnAddColumn(oSettings);
				fn(target, def);
...
function _fnAddData(settings, dataIn, tr, tds) {
	...
	if (id !== void 0) settings.aIds[id] = rowModel;
...
function _fnGetCellData(settings, rowIdx, colIdx, type) {
	...
	if (type === "filter") {
		...
		if (formatters[col.sType]) cellData = formatters[col.sType](cellData);
	}
	return cellData;
}
/**
...
function _fnInvalidate(settings, rowIdx, src, colIdx) {
	...
	if (src === "dom" || (!src || src === "auto") && row.src === "dom") row._aData = _fnGetRowElements(settings, row, colIdx, colIdx === void 0 ? void 0 : row._aData).data;
	else {
		var cells = row.anCells;
		var display = _fnGetRowDisplay(settings, rowIdx);
		if (cells) if (colIdx !== void 0) _fnWriteCell(cells[colIdx], display[colIdx]);
		else for (i = 0, iLen = cells.length; i < iLen; i++) _fnWriteCell(cells[i], display[i]);
	}
	var cols = settings.aoColumns;
	if (colIdx !== void 0) {
	...
	var cellProcess = function(cell) {
		if (colIdx === void 0 || colIdx === i) {
			...
			if (col && col._bAttrSrc) {
				_fnSetObjectDataFn(col.mData._)(d, contents);
...
function _fnBuildHead(settings, side) {
	...
	if (side === "header" || _pluck(settings.aoColumns, titleProp).join("")) {
		...
		if (row.length === 1) {
			...
			for (i = cellCount, iLen = columns.length; i < iLen; i++) $("<th/>").html(columns[i][titleProp] || "").appendTo(row);
		}
	}
...
function _fnDraw(oSettings, ajaxComplete) {
	_fnStart(oSettings);
	if (_fnCallbackFire(oSettings, "aoPreDrawCallback", "preDraw", [oSettings]).indexOf(false) !== -1) {
...
function _fnReDraw(settings, holdPosition, recompute) {
	...
	else _fnLengthOverflow(settings);
...
function _emptyRow(settings) {
	...
	if ((dataSrc === "ssp" || dataSrc === "ajax") && !settings.json) zero = oLang.sLoadingRecords;
...
function _layoutItems(row, align, items) {
	...
	if ($.isPlainObject(items)) if (items.features) {
	...
	for (var i = 0; i < rows.length; i++) {
		...
		if (row.rowNum === rowNum) {
			if (align === "full" && row.full || (align === "start" || align === "end") && (row.start || row.end)) {
				if (!row[align]) row[align] = { contents: [] };
				return row;
			}
		}
	}
...
function _layoutArray(settings, layout, side) {
	...
	$.each(layout, function(pos, items) {
		...
		_layoutItems(_layoutGetRow(rows, rowNum, align), align, items);
	});
...
function _layoutResolve(settings, row) {
	...
	var resolve = function(item) {
		...
		for (var i = 0, iLen = line.length; i < iLen; i++) if (!line[i]) continue;
...
function _fnAddOptionsHtml(settings) {
	...
	if (settings.sDom) _fnLayoutDom(settings, settings.sDom, insert);
	else {
					...
					if (unique) {
						...
						if (isHeader) {
							if (columnDef.sTitle !== null && !columnDef.autoTitle) {
								if (titleRow === true && i === 0 || titleRow === false && i === rows.length - 1 || titleRow === i || titleRow === null) cell.innerHTML = columnDef.sTitle;
							}
						...
						if (!columnDef.ariaTitle) columnDef.ariaTitle = jqCell.attr("aria-label") || columnDef.sTitle;
						if (columnDef.className) jqCell.addClass(columnDef.className);
					}
					if ($(".dt-column-title", cell).length === 0) $(document.createElement(settings.columnTitleTag)).addClass("dt-column-title").append(cell.childNodes).appendTo(cell);
					if (settings.orderIndicators && isHeader && jqCell.filter(":not([data-dt-order=disable])").length !== 0 && jqCell.parent(":not([data-dt-order=disable])").length !== 0 && $(".dt-column-order", cell).length === 0) $(document.createElement(settings.columnTitleTag)).addClass("dt-column-order").appendTo(cell);
					var headerFooter = isHeader ? "header" : "footer";
					if ($("div.dt-column-" + headerFooter, cell).length === 0) $("<div>").addClass("dt-column-" + headerFooter).append(cell.childNodes).appendTo(cell);
				}
				for (l = 0; l < colspan; l++) {
					for (k = 0; k < rowspan; k++) {
...
function _fnBuildAjax(oSettings, data, fn) {
	...
	var baseAjax = {
		...
		error: function(xhr, error) {
			if (_fnCallbackFire(oSettings, null, "xhr", [
				oSettings,
				null,
				oSettings.jqXHR
			], true).indexOf(true) === -1) {
	...
	if (baseAjax.submitAs === "json" && typeof data === "object") {
		...
		if (!baseAjax.contentType) baseAjax.contentType = "application/json; charset=utf-8";
	}
	if (typeof ajax === "function") oSettings.jqXHR = ajax.call(instance, data, callback, oSettings);
	else if (ajax.url === "") {
		var empty = {};
...
function _fnAjaxUpdateDraw(settings, json) {
	...
	for (var i = 0, iLen = data.length; i < iLen; i++) _fnAddData(settings, data[i]);
...
function _fnAjaxDataSrcParam(settings, param, json) {
	...
	if (dataSrc && dataSrc[param]) return _fnGetObjectDataFn(dataSrc[param])(json);
	var old = "";
...
function _fnFilterCreateSearch(search, inOpts) {
	...
	if (options.smart) {
		var a = (search.match(/!?["\u201C][^"\u201D]+["\u201D]|[^ ]+/g) || [""]).map(function(word) {
			var negative = false;
			var m;
			if (word.charAt(0) === "!") {
				negative = true;
...
function _fnInitialise(settings) {
	...
	_fnLoadState(settings, init, function() {
		...
		_fnReDraw(settings);
		if (dataSrc != "ssp" || deferLoading) if (dataSrc == "ajax") _fnBuildAjax(settings, {}, function(json) {
...
function _fnFeatureHtmlTable(settings) {
	...
	if (footer) scroller.append($(_div, { "class": classes.footer.self }).css({
		overflow: "hidden",
		border: 0,
		width: scrollX ? size(scrollX) : "100%"
	...
	if (settings.aiDisplay.length) {
		...
		if (firstTr) {
			...
			for (var i = 0; i < colSizes.length; i++) {
				...
				if (colEl.style.width.replace("px", "") !== colSizes[i].width) {
...
function _fnCalculateColumnWidths(settings) {
	...
	var longestData = [];
	for (i = 0; i < visibleColumns.length; i++) longestData.push(_fnGetWideStrings(settings, visibleColumns[i]));
	if (longestData.length) for (i = 0; i < longestData[0].length; i++) {
		...
		for (j = 0; j < visibleColumns.length; j++) {
			...
			var text = longest + (column.sContentPadding || (scrollX ? "-" : ""));
...
function _fnSortInit(settings) {
	...
	if (settings.orderHandler) _fnSortAttachListener(settings, target, target === settings.nTHead ? "tr" + notSelector + " th" + notSelector + ", tr" + notSelector + " td" + notSelector : "th" + notSelector + ", td" + notSelector);
	var order = [];
	...
	_fnBindAction(node, selector, function(e) {
		...
		if (columns.length) {
			for (var i = 0, iLen = columns.length; i < iLen; i++) {
				if (_fnSortAdd(settings, columns[i], i, e.shiftKey) !== false) run = true;
...
function _fnSortResolve(settings, nestedSort, sort) {
	var push = function(a) {
		if ($.isPlainObject(a)) {
			if (a.idx !== void 0) nestedSort.push([a.idx, a.dir]);
			else if (a.name) {
				var idx = _pluck(settings.aoColumns, "sName").indexOf(a.name);
				if (idx !== -1) nestedSort.push([idx, a.dir]);
			}
		} else nestedSort.push(a);
	};
	if ($.isPlainObject(sort)) push(sort);
	...
	for (i = 0; i < nestedSort.length; i++) {
		...
		if (aoColumns[srcCol]) {
			...
			for (k = 0, kLen = aDataSort.length; k < kLen; k++) {
				...
				if (nestedSort[i]._idx === void 0) nestedSort[i]._idx = aoColumns[iCol].asSorting.indexOf(nestedSort[i][1]);
...
function _fnLoadState(settings, init, callback) {
	...
	if (state !== void 0) _fnImplementState(settings, state, callback);
	return true;
}
function _fnImplementState(settings, s, callback) {
	...
	if (_fnCallbackFire(settings, "aoStateLoadParams", "stateLoadParams", [settings, s]).indexOf(false) !== -1) {
		settings._bLoadingState = false;
		callback();
		return;
	}
	...
	if (s.length !== void 0) if (api) api.page.len(s.length);
	else settings._iDisplayLength = s.length;
	if (s.start !== void 0) if (api === null) {
	...
	if (s.order !== void 0) {
		...
		$.each(s.order, function(i, col) {
			...
			if (typeof col[0] === "string") {
				...
				if (idx < 0) return;
				set[0] = idx;
			} else if (set[0] >= columns.length) return;
			settings.aaSorting.push(set);
		});
	}
	if (s.search !== void 0) $.extend(settings.oPreviousSearch, s.search);
	if (s.columns) {
		var set = s.columns;
		...
		if (set.length === columns.length) {
			for (i = 0, iLen = set.length; i < iLen; i++) {
				var col = set[i];
				if (col.visible !== void 0) if (api) api.column(i).visible(col.visible, false);
				else columns[i].bVisible = col.visible;
				if (col.search !== void 0) $.extend(settings.aoPreSearchCols[i], col.search);
			}
...
function _fnRenderer(settings, type) {
	...
	if ($.isPlainObject(renderer) && renderer[type]) return host[renderer[type]] || host._;
...
function _fnListener(that, name, src) {
	...
	for (i = 0; i < src.length; i++) that.on(name + ".dt", src[i]);
}
/**
* Escape HTML entities in strings, in an object
*/
function _fnEscapeObject(obj) {
	if (DataTable.ext.escape.attributes) $.each(obj, function(key, val) {
...
function _api_find(src, name) {
	for (var i = 0, iLen = src.length; i < iLen; i++) if (src[i].name === name) return src[i];
	return null;
}
...
_Api.registerPlural = _api_registerPlural = function(pluralName, singularName, val) {
	...
	_Api.register(singularName, function() {
		...
		else if (ret instanceof _Api) return ret.length ? Array.isArray(ret[0]) ? new _Api(ret.context, ret[0]) : ret[0] : void 0;
		return ret;
	});
};
/**
...
var __table_selector = function(selector, a) {
	if (Array.isArray(selector)) {
		...
		selector.forEach(function(sel) {
			_fnArrayApply(result, __table_selector(sel, a));
		});
...
_api_register("page.len()", function(len) {
	if (len === void 0) return this.context.length !== 0 ? this.context[0]._iDisplayLength : void 0;
	return this.iterator("table", function(settings) {
...
var _selector_run = function(type, selector, selectFn, settings, opts) {
	...
	if (ext.length) for (i = 0, iLen = ext.length; i < iLen; i++) out = ext[i](settings, opts, out);
	return _unique(out);
};
...
var _selector_row_indexes = function(settings, opts) {
	...
	if (_fnDataSource(settings) == "ssp") return search === "removed" ? [] : _range(0, displayMaster.length);
	if (page == "current") for (i = settings._iDisplayStart, iLen = settings.fnDisplayEnd(); i < iLen; i++) a.push(displayFiltered[i]);
	...
	} else if (order == "index" || order == "original") for (i = 0, iLen = settings.aoData.length; i < iLen; i++) {
...
var __row_selector = function(settings, selector, opts) {
	...
	var run = function(sel) {
		...
		if (sel.nodeName) {
			...
			if (rowIdx !== void 0) return aoData[rowIdx] && aoData[rowIdx].nTr === sel ? [rowIdx] : [];
...
_api_registerPlural("rows().remove()", "row().remove()", function() {
	this.iterator("row", function(settings, row) {
		...
		if (id !== void 0) delete settings.aIds[id];
		data[row] = null;
	});
	return this;
});
...
_api_register("row().data()", function(data) {
	var ctx = this.context;
	if (data === void 0) return ctx.length && this.length && this[0].length ? ctx[0].aoData[this[0]]._aData : void 0;
...
var __details_events = function(settings) {
	...
	if (_pluck(data, "_details").length > 0) {
		...
		api.on(destroyEvent, function(e, ctx) {
			if (settings !== ctx) return;
			for (var i = 0, iLen = data.length; i < iLen; i++) if (data[i] && data[i]._details) __details_remove(api, i);
		});
	}
};
...
_api_register(_child_mth, function(data, klass) {
	var ctx = this.context;
	if (data === void 0) return ctx.length && this.length && ctx[0].aoData[this[0]] ? ctx[0].aoData[this[0]]._details : void 0;
...
_api_register(_child_obj + ".isShown()", function() {
	var ctx = this.context;
	if (ctx.length && this.length && ctx[0].aoData[this[0]]) return ctx[0].aoData[this[0]]._detailsShow || false;
	return false;
});
var __re_column_selector = /^([^:]+)?:(name|title|visIdx|visible)$/;
var __columnData = function(settings, column, r1, r2, rows, type) {
	var a = [];
	for (var row = 0, iLen = rows.length; row < iLen; row++) a.push(_fnGetCellData(settings, rows[row], column, type));
	return a;
};
var __column_header = function(settings, column, row) {
	...
	if (row !== void 0) target = row;
		...
		for (var i = 0; i < header.length; i++) if (header[i][column].unique && $(".dt-column-title", header[i][column].cell).text()) target = i;
...
var __column_selector = function(settings, selector, opts) {
	...
	var run = function(s) {
		...
		if (match) switch (match[2]) {
			...
			case "name":
				if (!names) names = _pluck(columns, "sName");
				return names.map(function(name, i) {
					return name === match[1] ? i : null;
				});
			case "title":
				if (!titles) titles = _pluck(columns, "sTitle");
...
var __setColumnVis = function(settings, column, vis) {
	...
	if (vis === void 0) return col.bVisible;
	if (col.bVisible === vis) return false;
	if (vis) {
		var insertBefore = _pluck(cols, "bVisible").indexOf(true, column + 1);
		for (i = 0, iLen = data.length; i < iLen; i++) if (data[i]) {
			tr = data[i].nTr;
...
_api_registerPlural("columns().footer()", "column().footer()", function(row) {
	return this.iterator("column", function(settings, column) {
		if (!settings.aoFooter.length) return null;
...
_api_registerPlural("columns().visible()", "column().visible()", function(vis, calc) {
	...
	var ret = this.iterator("column", function(settings, column) {
		if (vis === void 0) return settings.aoColumns[column].bVisible;
		if (__setColumnVis(settings, column, vis)) changed.push(column);
	});
	if (vis !== void 0) this.iterator("table", function(settings) {
		_fnDrawHead(settings, settings.aoHeader);
		_fnDrawHead(settings, settings.aoFooter);
		if (!settings.aiDisplay.length) $(settings.nTBody).find("td[colspan]").attr("colspan", _fnVisibleColumns(settings));
...
var __cell_selector = function(settings, selector, opts) {
	...
	var run = function(s) {
		...
		if ($.isPlainObject(s)) return s.column !== void 0 && s.row !== void 0 && rows.indexOf(s.row) !== -1 ? [s] : [];
...
_api_register("cells()", function(rowSelector, columnSelector, opts) {
	if ($.isPlainObject(rowSelector)) if (rowSelector.row === void 0) {
		opts = rowSelector;
		rowSelector = null;
	} else {
		opts = columnSelector;
	...
	if (columnSelector === null || columnSelector === void 0) return this.iterator("table", function(settings) {
...
_api_register("cell().data()", function(data) {
	var ctx = this.context;
	var cell = this[0];
	if (data === void 0) return ctx.length && cell.length ? _fnGetCellData(ctx[0], cell[0].row, cell[0].column) : void 0;
...
_api_register("order()", function(order, dir) {
	...
	if (order === void 0) return ctx.length !== 0 ? ctx[0].aaSorting : void 0;
...
_api_register("search()", function(input, regex, smart, caseInsen) {
	var ctx = this.context;
	if (input === void 0) return ctx.length !== 0 ? ctx[0].oPreviousSearch.search : void 0;
	return this.iterator("table", function(settings) {
		if (!settings.oFeatures.bFilter) return;
		if (typeof regex === "object") _fnFilterComplete(settings, $.extend(settings.oPreviousSearch, regex, { search: input }));
		else _fnFilterComplete(settings, $.extend(settings.oPreviousSearch, {
			search: input,
...
_api_register("search.fixed()", function(name, search) {
	var ret = this.iterator(true, "table", function(settings) {
		...
		else if (search === void 0) return fixed[name];
...
_api_registerPlural("columns().search()", "column().search()", function(input, regex, smart, caseInsen) {
	return this.iterator("column", function(settings, column) {
		...
		if (input === void 0) return preSearch[column].search;
		if (!settings.oFeatures.bFilter) return;
		if (typeof regex === "object") $.extend(preSearch[column], regex, { search: input });
		else $.extend(preSearch[column], {
			search: input,
			regex: regex === null ? false : regex,
			smart: smart === null ? true : smart,
			caseInsensitive: caseInsen === null ? true : caseInsen
...
_api_register(["columns().search.fixed()", "column().search.fixed()"], function(name, search) {
	var ret = this.iterator(true, "column", function(settings, colIdx) {
		...
		if (!name) return Object.keys(fixed);
		else if (search === void 0) return fixed[name] || null;
		else if (search === null) delete fixed[name];
		else fixed[name] = search;
		return this;
	});
...
_api_register("state()", function(set, ignoreTime) {
	...
	return this.iterator("table", function(settings) {
		if (ignoreTime !== false) setMutate.time = +/* @__PURE__ */ new Date() + 100;
...
DataTable.use = function(arg1, arg2) {
	...
	if (module === void 0 && typeof type === "string") switch (type) {
		...
		case "datetime": return DataTable.DateTime;
		case "luxon": return __luxon;
		case "moment": return __moment;
		case "bootstrap": return __bootstrap || window.bootstrap;
		case "foundation": return __foundation || window.Foundation;
		default: return null;
	}
...
_api_register("i18n()", function(token, def, plural) {
	...
	if ($.isPlainObject(resolved)) resolved = plural !== void 0 && resolved[plural] !== void 0 ? resolved[plural] : plural === false ? resolved : resolved._;
...
$.extend(true, DataTable.ext.renderer, {
	pagingButton: { _: function(settings, buttonType, content, active, disabled) {
	...
	pagingContainer: { _: function(settings, buttons) {
		return buttons;
	} }
});
...
$.extend(true, DataTable.ext.renderer, {
	footer: { _: function(settings, cell, classes) {
		cell.addClass(classes.tfoot.cell);
	} },
	header: { _: function(settings, cell, classes) {
		cell.addClass(classes.thead.cell);
		if (!settings.oFeatures.bSort) cell.addClass(classes.order.none);
		var titleRow = settings.titleRow;
		var headerRows = cell.closest("thead").find("tr");
		var rowIdx = cell.parent().index();
		if (cell.attr("data-dt-order") === "disable" || cell.parent().attr("data-dt-order") === "disable" || titleRow === true && rowIdx !== 0 || titleRow === false && rowIdx !== headerRows.length - 1 || typeof titleRow === "number" && rowIdx !== titleRow) return;
...
DataTable.feature.register("info", function(settings, opts) {
	...
	var lang = settings.oLanguage, tid = settings.sTableId, n = $("<div/>", { "class": settings.oClasses.info.container });
	opts = $.extend({
...
DataTable.feature.register("search", function(settings, opts) {
	...
	var searchFn = function(event) {
		...
		if (val != previousSearch.search) _fnProcessingRun(settings, opts.processing, function() {
...
function _pagingDraw(settings, host, opts) {
	...
	if (activeEl !== void 0) host.find("[data-dt-idx=" + activeEl + "]").trigger("focus");
	if (buttonEls.length) {
		var outerHeight = $(buttonEls[0]).outerHeight();
		if (opts.buttons > 1 && outerHeight > 0 && $(host).height() >= outerHeight * 2 - 10) _pagingDraw(settings, host, $.extend({}, opts, { buttons: opts.buttons - 2 }));
	}
}
/**
...
function _pagingNumbers(page, pages, buttons, addFirstLast) {
	...
	else if (buttons === 3) if (page <= 1) numbers = [
		0,
		1,
		"ellipsis"
	];
...
DataTable.feature.register("pageLength", function(settings, opts) {
	...
	if (Array.isArray(menu[0])) {
		lengths = menu[0];
		language = menu[1];
	} else for (i = 0; i < menu.length; i++) if ($.isPlainObject(menu[i])) {
	...
	for (i = 0; i < lengths.length; i++) {
		...
		if (label === null) label = typeof language[i] === "number" ? settings.fnFormatNumber(language[i]) : language[i];
...
//#endregion
//#region node_modules/datatables.net-dt/js/dataTables.dataTables.mjs
var dataTables_dataTables_default = DataTable;
//#endregion
export { dataTables_dataTables_default as default };
